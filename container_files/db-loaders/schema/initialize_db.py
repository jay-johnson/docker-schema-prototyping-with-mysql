...
sys.path.append("/opt/db-loaders")
...
parser.add_argument('-e', help='Initializing Environment <Local|Debug|Test|Live>', required=True, dest='env')
parser.add_argument('-d', '--debug', help='Debug Flag')
args            = parser.parse_args()

db_config       = "/opt/db-loaders/configs/db.json"
env_name        = "Local"
debug           = False
...
if len(all_db_json["Database Applications"]) == 1:
    db_json = all_db_json["Database Applications"][0]
else:
...
results         = handle_build_initial_records(db_json, "/opt/db-loaders/data/", env_name)
if results["Status"] == "SUCCESS":
    lg("DB Records created successfully", 5)
else:
    lg("ERROR: Failed to create DB Records Error(" + str(results["Error"]) + ")", 0)
